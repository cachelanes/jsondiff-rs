use super::array::{diff_arrays_as_multiset, diff_arrays_as_set, diff_arrays_ordered};
...
pub fn diff_values(left: &Value, right: &Value, path: &JsonPath, config: &DiffConfig) -> Vec<DiffOp> {
    ...
    if left.is_array() && right.is_array() {
        ...
        return match config.array_mode {
            ...
            ArrayCompareMode::Set => diff_arrays_as_set(&left_slice, &right_slice, path, config),
            ArrayCompareMode::MultiSet => {
                diff_arrays_as_multiset(&left_slice, &right_slice, path, config)
            }
        };
    }
